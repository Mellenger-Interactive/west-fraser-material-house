...
- **Explode:** `updateParts()` moves products listed in `SPREAD` (`main.js`) as assemblies. An assembly is a set of touching parts of one product (union-find at startup). It lifts like its product, slides straight out from the house along its main horizontal axis until it clears every other exploded part by `EXPLODE_GAP`, and spreads its parts about its own centre by the product's factor. Long members stretch by the same factor. Everything else still uses `looseOffset()` (split at x 0.5 / z 0). Sasha's tuned values: `deck` 0.8, `EXPLODE_GAP` 0.7. Assemblies are computed in `SPREAD` order, and each one clears only the assemblies before it.
