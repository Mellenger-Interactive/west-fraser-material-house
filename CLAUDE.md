...
- **Explode:** `updateParts()` moves products listed in `SPREAD` (`main.js`) as assemblies: parts of one product within `join` of each other (per axis; default touching, union-find at startup) that share `userData.above` / `below` / `assembly` / `storey`. `plates` group per course, `webstock` / `rim` / `floor` per floor, `framing` into wall framing per storey, I-joist top and bottom flanges per floor, and the porch roof framing (`assembly: 'porch-roof'`). Parts spread about the assembly's own centre by `spread` (`plan: true` on `framing`: plan only, since wall layers stack by lift). Lift: the product's `looseOffset()` lift, or `above`'s lift + `EXPLODE_GAP`, or halfway between `above` and `below`, plus, for `userData.storey = 1` parts (the upper `floor()`, the upper walls and the main roof, tagged by `storey2()` in `model.js`), the ground storey's top-plate lift. Each floor then stacks bottom flange → web → top flange → subfloor, each wall floor → bottom plate → studs → top plate, and the 2nd storey sits above the ground storey. `slide: true` (deck, last in `SPREAD`): long members stretch by the spread factor, and the assembly slides straight out from the house until it clears every other exploded part (converted and loose) by `EXPLODE_GAP`. In place (everything else): no stretch or slide. It clears only the assemblies placed before it (earlier products, and earlier assemblies of its own product); if it overlaps, the whole assembly spreads further (up to `MAX_SPREAD`) or else lifts (up to `MAX_LIFT`), and if it still can't clear it keeps its normal lift and logs a `console.warn`. So each product converted later must clear the ones already done; overlaps with products not yet converted (LVL, trusses, wall sheathing, roof sheathing, MDF) are expected until they're converted. Sasha's tuned values: `deck` 0.8, `EXPLODE_GAP` 0.7; everything else is 0.5 and untuned. At the framing commit, all converted groups sit at 0.5, except the porch roof framing (+0.1 lift).
