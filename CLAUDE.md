...
- **Explode:** `updateParts()` moves products listed in `SPREAD` (`main.js`) as assemblies. An assembly is a set of touching parts of one product (union-find at startup). It lifts like its product, slides straight out from the house along its main horizontal axis until it clears every other exploded part by `EXPLODE_GAP`, and spreads its parts about its own centre by the product's factor. Long members stretch by the same factor. `RADIAL` products (`plates`, `webstock`) wrap or span the house: they spread about the house centre in plan with no stretch or slide, and any piece that would still overlap an exploded part is nudged further out by `NUDGE_GAP`. Parts can carry `userData.above` / `below` (plates in `wall()`): they lift `above`'s lift + `EXPLODE_GAP`, or halfway between `above` and `below`, so walls stack floor → bottom plate → studs → top plate. Everything else still uses `looseOffset()` (split at x 0.5 / z 0). Products are placed in `SPREAD` order, each clearing the final exploded boxes (`final`) of those before it and the loose positions of the rest. Sasha's tuned values: `deck` 0.8, `EXPLODE_GAP` 0.7; `plates` and `webstock` 0.5 are untuned. 
