...
- **Explode:** `updateParts()` moves products listed in `SPREAD` (`main.js`) as assemblies. An assembly is a set of parts of one product within `join` of each other (per axis; default touching, union-find at startup): `plates` join per course, `webstock` and `rim` per floor. Its parts spread about the assembly's own centre by `spread`. It lifts like its product, or by `userData.above` / `below` (plates in `wall()`: `above`'s lift + `EXPLODE_GAP`, or halfway between the two), so walls stack floor → bottom plate → studs → top plate. `slide: true` (deck): long members stretch by the spread factor, and the assembly slides straight out from the house along its main horizontal axis until it clears every other exploded part by `EXPLODE_GAP`. In place (everything else in `SPREAD`): no stretch or slide; if any part would overlap another exploded part, the whole assembly spreads further (up to `MAX_SPREAD`) or else lifts, so its pieces never separate. Products are placed in `SPREAD` order, each clearing the final exploded boxes (`final`) of those before it and the `looseOffset()` positions (split at x 0.5 / z 0) of products not yet in `SPREAD`. Sasha's tuned values: `deck` 0.8, `EXPLODE_GAP` 0.7. `plates`, `webstock` and `rim` are 0.5 and untuned; the upper-storey bottom plate course grows to 1.1 to clear, which Sasha accepted.
